...
def sampling(args):
    z_mean, z_sigma = args
    epsilon = K.random_normal(shape=K.shape(z_sigma), mean=0.,
                              stddev=0.33)  # originally stddev=1., but this creates values in between -3, 3
...
def build_vae_world_model(img_shape=(32, 32, 3), latent_size=16, opt='adam', loss='mse', conv_layers=4,
                          initial_filters=4):
    Z_DIM = latent_size
