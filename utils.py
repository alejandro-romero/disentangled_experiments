...
def loadAndResizeImages2(path, names=[], preprocessors=[], load_alpha=False):
    ...
    for name in names:
        if '.png' in name and load_alpha:
            try:
                img = imread(os.path.join(path, name), pilmode='RGBA')
                # imwrite("tmp/{}.png".format("lectura" + str(name)), img)
            except TypeError:
                img = imread(path / name, pilmode='RGBA')
        else:
            try:
                img = imread(os.path.join(path, name), pilmode='RGB')
                # imwrite("tmp/{}.png".format("lectura" + str(name)), img)
            except TypeError:
                img = imread(path / name, pilmode='RGB')
    ...
    # imwrite("tmp/{}.png".format("lectural"), np_images[0])
    # imwrite("tmp/{}.png".format("lecturan"), np_images[1])
